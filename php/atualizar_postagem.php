...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_FILES["imagem"]) && $_FILES["imagem"]["error"] === 0) {
        ...
        if (move_uploaded_file($_FILES["imagem"]["tmp_name"], $novoCaminhoImagem)) {
            ...
            $sql = "UPDATE postagens SET titulo = ?, conteudo = ?, caminho_imagem = ?, data_publicacao = ?, tag = ? WHERE id_postagem = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sssssi", $novoTitulo, $novoConteudo, $novoCaminhoImagem, $dataAtualizacao, $tag, $postagemId);
        ...
        $sql = "UPDATE postagens SET titulo = ?, conteudo = ?, data_publicacao = ?, tag = ? WHERE id_postagem = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssi", $novoTitulo, $novoConteudo, $dataAtualizacao, $tag, $postagemId);

        if ($stmt->execute()) {
            echo "Postagem atualizada com sucesso.\n redirecionando...";
            echo "<script>setTimeout(function(){ window.location.href = '../pages/postagens.php?username=" . urlencode($user) . "'; }, 2000);</script>";
        } else {
            echo "Erro ao atualizar a postagem: " . $stmt->error;
        }
    }
